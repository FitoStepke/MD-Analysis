...
"""
flagsOld = numpy.zeros((lenSel, 2)) # Initializing a zeros array for flagsOld
...
for i, frame in enumerate(traj):
    ...
    flags = numpy.where(numpy.logical_and(posArray[:,2] < upZ, posArray[:,2] > loZ), 3*numpy.ones(lenSel), flags) # Slab
...
#    print(f'{flagsOld}   {flags}') A remnant from desperate times.
    ...
    try:
        diffArray = numpy.flatnonzero(numpy.all(numpy.not_equal(flagsOld[:,1:], flags[:,numpy.newaxis]), axis = 1)) # Explained a while back. Grabbing the Arrering indices to update them in flagsOld
    except:
        continue
    counter[1] += len(numpy.flatnonzero(numpy.all(numpy.equal(numpy.hstack((flagsOld[diffArray], flags[diffArray, numpy.newaxis])), [1,3,5]), axis = 1))) # Update up direction counter
# Maybe directly modify the permArray instead, bypassing the counter variable?
    counter[0] += len(numpy.flatnonzero(numpy.all(numpy.equal(numpy.hstack((flagsOld[diffArray], flags[diffArray, numpy.newaxis])), [5,3,1]), axis = 1))) # Update down direction counter
